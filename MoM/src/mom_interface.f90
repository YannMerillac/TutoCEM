subroutine build_impedance_matrix(w_z,k,a,Zmn)
    use KernelFunctions
    implicit NONE
    double precision, INTENT(IN), dimension(:) :: w_z
    double precision, INTENT(IN) :: k,a
    double complex, INTENT(INOUT), dimension(:,:) :: Zmn
    double complex :: Amn,Phimn
    integer :: i_n, i_m, n
    n = size(w_z)-2
    call set_kernel_parameters(w_z,k,a)
    do i_m = 1,n
        do i_n = 1,n
...
subroutine build_sources(w_z,k,a,i_source,V)
    use KernelFunctions
    implicit NONE
    double precision, INTENT(IN), dimension(:) :: w_z
    double precision, INTENT(IN) :: k,a
